  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-10 bg-gradient-to-b from-white to-gray-50">
      <div className="w-full max-w-md bg-white border border-gray-100 rounded-3xl shadow-xl shadow-gray-200/60 p-8">
        ...
        <h1 className="text-2xl font-extrabold tracking-tight text-navy mb-6">Create a project</h1>

        {!joinCode ? (
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-mid mb-1 block">Project name</label>
              <input
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            <p className="text-gray-mid text-sm">Share this code with your teammates:</p>
            <div className="bg-orange-light text-orange text-2xl font-extrabold tracking-widest px-6 py-3 rounded-2xl">
              {joinCode}
            </div>
            <button
              onClick={handleCopy}
              className="text-sm font-semibold text-navy border border-gray-200 rounded-xl px-4 py-2 hover:bg-gray-50 transition-all"
            >
              {copied ? "Copied!" : "Copy code"}
            </button>
            <button
              onClick={handleGoToProject}
              className="bg-orange text-white font-bold py-2.5 px-6 rounded-xl mt-2 shadow-md shadow-orange/20 hover:bg-orange-dark hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150"
            >
              Go to project →
            </button>
          </div>
